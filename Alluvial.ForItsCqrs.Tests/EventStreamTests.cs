...
new StorableEvent { StreamName = typeof(AggregateA).Name, Type=typeof(AggregateA.EventType7).Name, AggregateId = AggregateId4, SequenceNumber=16, Id = 	46,	Body = "{}" },
            ...
            //{
            //    var stream = EventStream.AllChanges("My Stream ID", () => storableEvents);
            //    var aggregateIds = new HashSet<Guid>();
            //    var streamCatchup = StreamCatchup.Create(stream);
            //    streamCatchup.Subscribe(async b =>
            //    {
            //        foreach (var id in b.Select(item => item.AggregateId))
            //        {
            //            aggregateIds.Add(id);
            //        }
            //    });
            //    await streamCatchup.RunUntilCaughtUp();
            //    aggregateIds.Should().HaveCount(4);
            //}
